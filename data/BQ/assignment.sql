...
CREATE TABLE IF NOT EXISTS `gcp-healthcare-project.gold_dataset.financial_metrics` AS
SELECT 
...
CREATE TABLE IF NOT EXISTS `gcp-healthcare-project.gold_dataset.payor_performance` AS
SELECT 
    c.PayorID,
    c.PayorType,
    COUNT(DISTINCT c.Claim_Key) AS TotalClaims,
...
FROM `gcp-healthcare-project.silver_dataset.claims` c
